...
class Mage_CatalogSearch_Model_Resource_Fulltext extends Mage_Core_Model_Resource_Db_Abstract
{
				...
				if(count($word1)>0){
					...
					for($i = 0;$i<$count;$i++){
						...
						if($j == 1){
							...
							$wordss = array_rand($word1,$j);
							if($this->getSearchProduct($wordss)){
	...
	* return boolean
	*/
	public function getWordProduct($word){
		$read = Mage::getSingleton("core/resource")->getConnection('core_read');
		$storeid =  Mage::app()->getStore()->getStoreId();
		$word = $word;
		$sql = "select `product_id` from `catalogsearch_fulltext`  where (`store_id`=".$storeid.") AND ( `data_index` LIKE '%".$word."%' )" ;
		$result = '';
	...
	* return boolean
	*/
	public function getSearchProduct($words){
		$read = Mage::getSingleton("core/resource")->getConnection('core_read');
		$storeid =  Mage::app()->getStore()->getStoreId();
		$words = $words;;
		$where = '';
		...
		{	++$n;
			if($n == 1){
				$where .= "`data_index` LIKE '%".$word."%' ";
			}else{
				$where .= " AND `data_index` LIKE '%".$word."%' ";
			}
		}
		$sql = "select `product_id` from `catalogsearch_fulltext`  where (`store_id`=".$storeid.") AND (".$where.")" ;
		$result = '';
		$result = $read->fetchAll($sql);
		if(count($result)>0){
			return true;
