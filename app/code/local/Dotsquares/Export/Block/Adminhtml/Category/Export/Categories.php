 ...
 */
class Dotsquares_Export_Block_Adminhtml_Category_Export_Categories extends Dotsquares_Export_Block_Adminhtml_Category_Export
{
	...
	public function getCsvData($opt = '') {
		if ($opt == 'all') {
			if (is_null($this->_csvData)) {
				$collection = Mage::getModel('dotsquares_core/attribute')->getCollection()
						->addCatalogEav()
			...
			return Mage::helper('dotsquares_core/category')->getAttributes();
		}
	}
