...
class Dotsquares_Import_Adminhtml_Import_CategoryController extends Mage_Adminhtml_Controller_Action
{
	...
	public function categoriesAction() {
		if ($this->getRequest()->isPost()) {
			...
			try {
				...
				$helper = Mage::helper('dotsquares_import/category');
				...
				if (is_array($catid)) {
					$error = Mage::helper('dotsquares_import')->__('Path provided is not a valid one.');
				} else {
					...
					foreach($attributes as $attribute) {
						...
						if (isset($attribute['required'])) {
							if (strtolower($attribute['required']) == 'yes') {
								if (!$fieldvalue) {
									$error = Mage::helper('dotsquares_import')->__('Please provide the value for "%s"', $attribute['field']);
									break;
								}
							}
						}
				...
				if ($error) {
					...
					$category->save();
					$result['message'] = Mage::helper('dotsquares_import')->__('Imported the category "%s" successfully.', $category->getName());
				}
			} catch (Exception $e) {
				$result['error'] = $e->getMessage();
			}
		} else {
			$result['error'] = Mage::helper('dotsquares_import')->__('Invalid request');
		}
